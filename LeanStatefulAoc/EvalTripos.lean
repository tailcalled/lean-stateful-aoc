...
/-- Transitivity: the composite realizer `r := S (S (K seq) r₁) (K r₂)` satisfies
`r · a ↠ seq · (r₁ · a) · (K · r₂ · a)`, running `r₁` on `a` then `r₂` on the result —
exactly `Produces.bind`. -/
theorem EEntails.trans {P Q R : EProp} (hPQ : P ⊨ Q) (hQR : Q ⊨ R) : P ⊨ R := by
  obtain ⟨r₁, h₁⟩ := hPQ
  obtain ⟨r₂, h₂⟩ := hQR
  refine ⟨S ⬝ (S ⬝ (K ⬝ seq) ⬝ r₁) ⬝ (K ⬝ r₂), fun h a ha => ?_⟩
  -- produce R from the literal seq form, with continuation k = K · r₂ · a
  have hbind : R.Produces h (seq ⬝ (r₁ ⬝ a) ⬝ (K ⬝ r₂ ⬝ a)) := by
    refine EProp.Produces.bind (h₁ h a ha) (fun h' v hqv => ?_)
  ...
  refine ⟨h', v, ?_, hRv⟩
  -- transport the seq-form evaluation back along the S/K head reductions of  r · a
  obtain ⟨hX, fvX, hseqhead, mX, hap⟩ := Evaluates.app_inv hevseq
  obtain ⟨heq, feq⟩ := Evaluates.deterministic hseqhead Evaluates.seqVal
  rw [heq, feq] at hap
  -- head:  S (K seq) r₁ · a  ↠  seq · (r₁ · a)
  have hHead : Evaluates h (S ⬝ (K ⬝ seq) ⬝ r₁ ⬝ a) h (seq ⬝ (r₁ ⬝ a)) :=
    Evaluates.headS (Evaluates.mk_app (Evaluates.headK Evaluates.seqv)
